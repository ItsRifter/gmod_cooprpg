...
function PANEL:Setup(group,id)
	...
	self.value = HL2C_Ach:GetProgress(id)
	self.max = ach.Max or 1
	if ach.Secret then self.Secret = true end
	if HL2C_Ach:HasAchievement(id) then self.value = self.max end
	
	self.Name = New_ThemeText(self,tall * 0.82, tall * 0.03,translate.Get(ach.Name),"Font2_Small",0,0)
	...
	if self.value == self.max then
		local txt = New_ThemeTextMulti(self,tall * 0.82, tall * 0.28,translate.Get(ach.Desc),"Font2_Micro",0,0,2)
		Icon:SetImage(ach.Mat)
	else
		local txt = New_ThemeText(self,tall * 0.82, tall * 0.28,"???","Font2_Micro",0,0)
		Icon:SetImage("materials/hl2c/icons/locked.png")
	end
...
local function New_AchievementPanel(parent,achx, achy,group,id)
...
function PANEL:Setup()
	...
	for k, v in HL2C_Ach:GetGroups() do
		local achx = tall * 0.01
		local txt = New_ThemeText(self.AchPanel,achx, achy,translate.Get(v.Name),"Font_Normal",0,0)
		achy = achy + wide2 * 0.035
		local count = 0
		for kk, vv in HL2C_Ach:GetGroupAchievements(k)do 
			if vv.Secret and not HL2C_Ach:HasAchievement(kk) then continue end
			...
			New_AchievementPanel(self.AchPanel,achx, achy,k,kk)
...
function PANEL:Paint()
	...
	draw.RoundedBox( 8, 0, 0, wide , tall, Theme.backcol2)
	
	return true
end
vgui.Register( "QMenu_Achievement", PANEL, "Panel" )

function HL2C_Client:CreateAchievement(parent)
	local element = vgui.Create("QMenu_Achievement",parent)
