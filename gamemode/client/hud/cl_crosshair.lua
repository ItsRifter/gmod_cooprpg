...
function Crosshair.Draw()
	...
	if HL2C_Client.Config.NewCross and !IsValid(GAMEMODE.qMenuFrame) then
		if cross[HL2C_Client.Config.CrossType] then
			--This code gets the users actual aim spot instead of putting it in middle of screen
			if HL2C_Client.Config.DynamicCross and not LocalPlayer():InVehicle() then
			...
			cross[HL2C_Client.Config.CrossType](midx,midy,size)
	    end
