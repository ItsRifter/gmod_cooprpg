...
if CLIENT then 
	function HL2C_Global:GetLvlAccess()
...
elseif SERVER then
	function HL2C_Global:GetLvlAccess(ply)
...
function HL2C_Global:GetModelList(lvl)
	modlist = {}
	local i = 1
	for k, v in SortedPairsByMemberValue(HL2C_Global.PlayerModels, "index", false) do
		if lvl >= v.lvl then
			modlist[i] = k
			i = i + 1
		end
...
function HL2C_Global:GetModelGroups(group,lvl)
	if !HL2C_Global.PlayerModels[group] then return nil end
	modlist = {}
	local i = 1
	for k, v in SortedPairsByMemberValue(HL2C_Global.PlayerModels[group].group, "index", false) do
		if lvl >= v.lvl then
			modlist[i] = k
			i = i + 1
		end
...
function HL2C_Global:GetModelFromGroup(group)
	if !HL2C_Global.PlayerModels[group] then return nil end
	for k, v in SortedPairsByMemberValue(HL2C_Global.PlayerModels[group].group, "index", false) do
		return v.mdl
	end
...
function HL2C_Global:GetModelFromName(group,name,lvl)
	if !HL2C_Global.PlayerModels[group] then return nil end
	if lvl < HL2C_Global.PlayerModels[group].lvl then return nil end
	for k, v in pairs(HL2C_Global.PlayerModels[group].group) do
		if k == name then 
			if lvl >= v.lvl then
				return v.mdl
			end
